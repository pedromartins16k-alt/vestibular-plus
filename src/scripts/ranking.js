...
function iniciais(p) {
  return nomeExibicao(p).trim().charAt(0).toUpperCase();
}
function renderPodio(top3) {
  ...
  podioEl.innerHTML = top3.map((p, i) => `
    <div class="card podio-item ${classes[i]}">
      ...
      <div class="podio-avatar">${iniciais(p)}</div>
      <div class="podio-nome">${nomeExibicao(p)}</div>
...
function renderLista(perfis, userId) {
  listaEl.innerHTML = perfis.map((p, i) => `
    <div class="card ranking-row ${p.id === userId ? 'voce' : ''}">
      ...
      <div class="ranking-avatar">${iniciais(p)}</div>
      <div class="ranking-info">
        <div class="ranking-nome">${nomeExibicao(p)}${p.id === userId ? ' (você)' : ''}</div>
